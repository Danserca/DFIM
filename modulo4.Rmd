...
paquetes <- c(
  "class", "pscl", "MASS", "lmtest", "sandwich", "tseries", "raster", 
  "sf", "rgdal", "ggplot2", "caret", "hot.deck", "broom", "beepr", 
  "foreign", "readr", "dplyr", "janitor", "survey", "openxlsx", 
  "tidyverse", "date", "data.table", "questionr", "readxl", "writexl", 
  "PracTools", "haven", "knitr", "kableExtra"
)
...
```
En este paso, se limpian y organizan los datos de población migrante, y se preparan para su posterior análisis.    

2.2 Tasas de Homicidios por Municipio     
Se carga otro conjunto de datos que contiene las tasas de homicidios por cada cien mil habitantes en municipios mexicanos, para el período 2015-2017.    
```{r, eval=FALSE}
# Leer el archivo Excel y limpiar los nombres de las columnas
...
```
2. Carga y Procesamiento de Datos  
2.1 Establecer Causas de Defunción por Homicidio  
...
```
2.3 Generar Variables y Agrupar Datos   
...
```
2.4 Carga de Otros Datos: Índice de Marginación e Intensidad Migratoria   
...
# Índice de marginación normalizado 2015 (carencias poblacionales) por localidad, municipio y entidad
...
imm <- read_xlsx(archivo_imm, sheet = "IMM_2015") %>% 
  clean_names() %>% 
  rename(id_mun = cve_mun)

# Intensidad migratoria municipal. Fuente CONAPO
iaim_2020 <- read_csv("https://conapo.segob.gob.mx/work/models/CONAPO/IIM/iim_base2020m.csv")
iaim_2020 <- iaim_2020  %>% 
  clean_names() %>% 
  mutate(id_mun = ifelse(nchar(cve_geo) == 4, paste0("0", cve_geo), cve_geo))

```


2.5 Procesamiento de Datos de Población   
...
# Arreglar claves de los municipios
poblacion_municipal <- poblacion_municipal %>% 
  mutate(entidad = ifelse(nchar(clave_ent) == 1, paste0("0", clave_ent), clave_ent),
         id_mun = ifelse(nchar(clave) == 4, paste0("0", clave), clave))
...
Más grupos de edad...
2.6 Cálculo de Tasas Promedio de Homicidio  
...
tasas <- pob_anual %>%
  ...
  mutate(
    tasa_hom_100mil = round((homicidios / poblacion_anual) * 100000, 1),
    ent_regis = substr(id_mun, 1, 2)
  )

# Calcular tasas promedio suavizadas usando el método de Bayes Empírico
tasas_por_edo <- tasas  %>%
  group_by(ent_regis, anio) %>%
  mutate(
    mu = sum(homicidios) / sum(poblacion_anual),
    O_i = homicidios,
    P_i = poblacion_anual,
    n = n(),
    r_i = homicidios / poblacion_anual,
    variance = var(homicidios / poblacion_anual)
  ) %>%
  mutate(
    w_i = variance / (variance + mu / P_i),
    eb_bayes = w_i * r_i + (1 - w_i) * mu
  )

# Resumir tasas suavizadas por municipio
...
2.7 Integración de Datos y Proyección Geográfica  
...
flextable::save_as_docx(summary_flextable, path = "RESULTADOS_OK/summary_table_240620.docx")

```

1. Cálculo de los 5 Vecinos Más Cercanos
...
# Calcular los 5 vecinos más cercanos
mex5_nb <- knn2nb(knearneigh(mexico.coords, k = 5))

# Graficar los vecinos más cercanos
plot(mexico.geom, main = "k = 5", reset = FALSE)
plot(mex5_nb, mexico.coords, add = TRUE, col = 2, lwd = 1.5)
...
gls_model_2 <- gls(formula_1, data = data_mun15, weights = varIdent(form = ~ 1 | factor(cluster)))
weighted_model_2 <- lm(formula_2, data = data_mun15, weights = peso)
...
Se define una función para calcular las métricas y pruebas estadísticas de cada modelo, como el AIC, BIC, y pruebas de Breusch-Pagan y Jarque-Bera. También se calcula el índice de Moran si se proporcionan datos espaciales.
