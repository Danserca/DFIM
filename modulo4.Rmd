...
\[
\text{tasa\_dfi}_i = \beta_0 + \beta_1 \cdot \text{tasa\_promedio\_hom}_i + \beta_2 \cdot (\text{tasa\_promedio\_hom}_i)^2 + \beta_3 \cdot \text{salario\_mun\_mediano}_i + \beta_4 \cdot \text{presion\_demo}_i + \beta_5 \cdot \log(\text{pob\_promedio}_i) + \beta_6 \cdot \text{cluster}_i + \epsilon_i
\]
...
- \( \text{tasa\_dfi}_i \) es la tasa de personas en situación de DFI por cada cien mil habitantes.
- \( \log(\text{pob\_promedio}_i) \) es el logaritmo natural de la población promedio para la observación \( i \).

Incluye el término cuadrático de tasa_promedio_hom^2.
